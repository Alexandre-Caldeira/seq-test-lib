# =============================================================================
#
# EEG SEQUENTIAL PROTOCOL ANALYSIS PIPELINE (v3.1.0)
#
...
DEFAULT_CONFIG = {
    ...
    'FILTER_TYPE': 'custom',              # Bandpass filter type. Options: 'custom' or None.
    'CUSTOM_FILTER_LOW_CUT': 30.0,        # Lower cutoff frequency (Hz) for the bandpass filter
    'CUSTOM_FILTER_HIGH_CUT': 300.0,      # Upper cutoff frequency (Hz) for the bandpass filter
    'CUSTOM_FILTER_ORDER': 2,             # Order of the Butterworth filter
    
    'APPLY_NOTCH_FILTER': False,          # Whether to apply a notch filter. Options: True or False.
    'NOTCH_FREQ': 60.0,                   # Frequency to target with the notch filter (e.g., 50 or 60 Hz)
    'NOTCH_Q': 100,                       # Quality factor of the notch filter
    
    'ARTIFACT_REJECTION_TYPE': None,      # Artifact rejection method. Options: 'percentile' or None.
    'PERCENTILE_REMOVAL_AMOUNT': 15,       # Percentage of windows with the highest amplitude to discard
    
    'APPLY_MEAN_REMOVAL': True,           # Subtract the mean from each window. Options: True or False.
    'REMOVE_FIRST_TWO_WINDOWS': False,    # Discard the first two windows of each recording. Options: True or False.
...
def preprocess_data(data, fs, config):
    ...
    # Apply filters along the time axis (axis=1)
