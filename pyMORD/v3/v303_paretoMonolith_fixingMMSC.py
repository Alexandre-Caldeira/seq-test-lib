...
DEFAULT_CONFIG = {
    ...
    # --- 2. Performance Enhancements (Turned off by default) ---
    ...
    'ALPHA': 0.05,
    'FP_DESEJADO': 0.05,
    'METRICS_TO_RUN': ['MMSC'], #['MMSC', 'MCSM', 'D-aMSC', 'D-aCSM'],
    'TARGET_ELECTRODES': ['Fz'],
    
    # --- 4. Preprocessing Parameters (Integrated from reference) ---
    'FILTER_TYPE': 'custom',              # Options: 'custom' or None
    'CUSTOM_FILTER_LOW_CUT': 30.0,
    'CUSTOM_FILTER_HIGH_CUT': 300.0,
    'CUSTOM_FILTER_ORDER': 2,
    'APPLY_NOTCH_FILTER': False,          # Options: True or False
    'NOTCH_FREQ': 60.0,
    'NOTCH_Q': 100,
    'ARTIFACT_REJECTION_TYPE': None, # Options: 'percentile' or None
    'PERCENTILE_REMOVAL_AMOUNT': 5,
    'APPLY_MEAN_REMOVAL': True,           # Options: True or False
    'REMOVE_FIRST_TWO_WINDOWS': False,    # Options: True or False
...
def setup_accelerators(use_numba_flag):
    """FIXED: Silently defines functions based on whether Numba is enabled."""
    global msweep, dipolos_py, ETS_py, pareto_front_py

    def msweep_python(matrix, r): # ... (implementation unchanged)
        ...
        return A
    def dipolos_python(x): # ... (implementation unchanged)
        ...
        for i, j in itertools.combinations(range(n_channels), 2):
            y[:, idx] = x[:, i] - x[:, j]; idx += 1
        return y
    def ETS_python(ord_values, MM, alfa, NDC, vc_values): # ... (implementation unchanged)
        ...
        return 0, MM[-1]
    def pareto_python(points): # ... (implementation unchanged)
    ...
    if use_numba_flag and NUMBA_AVAILABLE:
        ...
        def msweep_njit(matrix, r): # ... (implementation unchanged)
            A = np.copy(matrix)
            for k in range(r):
                d = A[k, k];
                if np.abs(d) < 1e-12: d = 1e-12
        ...
        def dipolos_njit(x): # ... (implementation unchanged)
            n_samples, n_channels = x.shape
            n_combs = n_channels * (n_channels - 1) // 2
        ...
        def ETS_njit(ord_values, MM, alfa, NDC, vc_values): # ... (implementation unchanged)
            NDC = np.ceil(NDC); det = ord_values > vc_values
            consecutive_detections = 0
            for i, is_detected in enumerate(det):
                consecutive_detections = consecutive_detections + 1 if is_detected else 0
        ...
        def pareto_front_njit(points): # ... (implementation unchanged)
...
# --- Core Metric and Protocol Functions ---
...
def protocolo_deteccao_py(x, fs, parametros, metric_name): # ...
    ...
    for ii in range(num_param_sets):
        ...
        vc_for_ets = crit_matrix[0, MM - 2] # Critical value is constant across frequencies
        for ll in range(num_bins_protocol):
            dr[ll, ii], time[ll, ii] = ETS_py(ord_matrix[ll, MM - 2], MM, alfa_corr, NDC, vc_for_ets)
    return dr, time

# =============================================================================
# Section 3: Preprocessing and Main Execution
# =============================================================================
...
def preprocess_data(data, fs, config): # ...
    ...
    if config.get('ARTIFACT_REJECTION_TYPE') == 'percentile' and eeg_windows.shape[0] > 0:
...
def process_volunteer(args): # ...
    ...
    try:
        ...
        input_data = np.array([dipolos_py(processed_data[:, i, :]) for i in range(processed_data.shape[1])]).transpose(1,0,2) if metric in ['D-aMSC', 'D-aCSM'] else processed_data
...
def main(config):
    # FIXED: Print status only once from the main process
    ...
    # Silently set up accelerators for the main process
    ...
    for metric in config['METRICS_TO_RUN']:
        ...
        # --- Result aggregation and plotting (unchanged) ---
        ...
        if not Tdr_all: 
            print(f"Warning: No valid data for metric {metric} after processing.")
            continue
...
if __name__ == '__main__':
    ...
    final_config = load_config_from_yaml(args.config) if args.config and os.path.exists(args.config) else DEFAULT_CONFIG
    
    if final_config.get('USE_MULTIPROCESSING', False):
