...
for i = 1:numel(vaz_startWindows_per_stimuli)
    vaz_startWindows_per_stimuli{i} = 10:(dtl.zanoteliSuggestedMMax(i)-40);
end
% vaz_startWindows_per_stimuli = cell(size(dtl.zanoteliSuggestedMMax));
% for i = 1:numel(vaz_startWindows_per_stimuli)
%     vaz_startWindows_per_stimuli{i} = 120:10:(dtl.zanoteliSuggestedMMax(i)-90);
% end
...
head(results_table)
% writetable(results_table,'50db_cgst.csv')
%%
...
grid on
hold on
title(['Curva de Pareto Intensidade ', (Intensidade{1}), ' SPL'])
...
u = plot(best_tp,best_tp_time,'-o blue','Markersize',8,'linewidth',1.8) ;
...
% for ii = 1:size(idxs,1)
% 
%     [I] = find((tp_rate2 == tp_rate2(idxs(ii))) & (minimal_detection_time2==timeM(minimal_detection_time2(ii))));
%     I = I(1); 
% 
%     for jj = I  
%         text(tp_rate2(jj),minimal_detection_time2(idxs(ii))*.975,['\{' num2str(mmin(jj,1)) ',' num2str(mstep(jj,2)) '\}' ]);
%     end    
% end


legend([linha(1),linha(2),linha(3),linha(4),u],'(Zanotelli,2020)', ...
    '(Bazoni,2021)','(Antunes, 2019)','(Vaz, 2023)','ASSR-CGST')
% save(s,'-v7.3')