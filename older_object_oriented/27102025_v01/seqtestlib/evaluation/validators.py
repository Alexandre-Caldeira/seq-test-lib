...
class Evaluator:
    ...
    def _fit_or_set_threshold(self, data_source: dict, train_indices: np.ndarray, noise_bins: list, job: dict):
        ...
        if isinstance(self.tester.model, (DANNModel, LGBMModel)):
            ...
            X, y, domains = Evaluator._prepare_ml_data(
                data_source, train_indices, noise_bins, job, self.m_val, max_samples=max_samples
            )
            if X.shape[0] > 0:
    ...
    @staticmethod
    def _prepare_ml_data(data_source, indices, noise_bins, job, m_val, max_samples=None):
        ...
        for snr, snr_data in data_source.items():
            ...
            sig_feats = SimulatedLoader._create_sliding_windows(sig_ts, m_val)
            noise_feats = SimulatedLoader._create_sliding_windows(noise_ts, m_val)
        ...
        if max_samples is not None and X.shape[0] > max_samples:
            print(f"\nNOTE: Subsampling ML data from {X.shape[0]} to {max_samples} samples.")
            rng = np.random.default_rng(config.RANDOM_STATE)
            sample_indices = rng.choice(X.shape[0], max_samples, replace=False)
            X, y, domains = X[sample_indices], y[sample_indices], domains[sample_indices]
