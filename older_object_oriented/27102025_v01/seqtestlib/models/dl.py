...
class DANNModel(BaseDetector):
    ...
    def predict_score(self, X):
        """
        Predicts the anomaly score for the input data X.
        Includes robust reshaping to handle potential input dimension errors.
        """
        self.model.eval()
        with torch.no_grad():
            ...
            # Get the expected feature dimension from the model's first layer
            expected_features = self.model.feature_extractor[0].in_features
            
            # If the shape is incorrect (e.g., [1, N*D]), reshape it to [N, D]
            if x_tensor.shape[1] != expected_features:
                if x_tensor.shape[0] == 1 and x_tensor.shape[1] % expected_features == 0:
                    num_samples = x_tensor.shape[1] // expected_features
                    print(f"\n[DANNModel WARNING] Reshaping input in predict_score from {x_tensor.shape} to ({num_samples}, {expected_features})")
                    x_tensor = x_tensor.view(num_samples, expected_features)
                else:
                    # If it can't be fixed, raise a more informative error
                    raise ValueError(
                        f"Input shape mismatch in DANNModel.predict_score. "
                        f"Expected (_, {expected_features}), but got {x_tensor.shape}."
                    )
